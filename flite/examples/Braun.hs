...
equal [] [] = True;
equal [] (Cons y ys) = False;
equal (Cons x xs) [] = False;
...
upto m n = if equal m n then Cons m [] else Cons m (upto (inc m) n);
...
replicate n x = if (==) n 0 then [] else Cons x (replicate ((-) n 1) x);
