...
equalStrings []         []          =  True ;
equalStrings []         (Cons y ys)  =  False ;
equalStrings (Cons x xs) []          =  False ;
...
weights  =  Weights 1 (Cons (Pair "0" 1) (Cons (Pair "+" 0) (Cons (Pair "-" 0) []))) ;
...
main  =
  let {
    sig  =  Cons (Pair "+" (Pair 2 True))  (
            ...
            Cons (Pair "0" (Pair 0 False)) [] )) ;

    eqns  =  Cons (Pair (Fun "+" (Cons (Fun "0" []) (Cons (Var "X") [])))
                        (Var "X")) (
...
zipWithRemRight f []         []          =  Pair [] [] ;
zipWithRemRight f []         (Cons y ys)  =  Pair [] (Cons y ys) ;
zipWithRemRight f (Cons x xs) []          =  Pair [] [] ;
...
reduceSplit sig rule []                   =  Pair [] []  ;
reduceSplit sig rule (Cons (Pair l r) rs)  =
        let { reducedl  =  reduce (Cons rule []) l ; } in
  case reduceSplit sig rule rs of {
  Pair eqns rules ->
        case dis (null reducedl)
             (reducible (Cons (Pair l r) []) (left rule)) of {
        True  -> Pair eqns
                  (Cons (Pair l r) rules) ;
...
orderLex f []         []          =  False ;
orderLex f []         (Cons y ys)  =  False ;
orderLex f (Cons x xs) []          =  False ;
...
unify t1 t2  =  unifyWith (Cons t1 []) (Cons t2 []) [] ;
...
unifyWith (Cons (Var v) ts1) (Cons t ts2) sub  =
        ...
        False -> case equalTerms vi (Var v) of {
           True  ->  case elemBy equalStrings v (vars ti) of {
                     True  -> Nothing ;
                     False -> unifyWith ts1 ts2 (substAdd (Cons (Pair v ti) []) sub) ;
                     } ;
...
match t1 t2  =  matchWith (Cons t1 []) (Cons t2 []) [] ;
...
reduceAtWith t p (Pair l r)  =
  ...
  Nothing -> [] ;
  Just sub -> Cons (placeAt (subst sub r) p t) [] ;
  } ;
...
positions (Var v)     =  Cons [] [] ;
positions (Fun f ts)  =  Cons [] (concatMap (argPositions ts)
