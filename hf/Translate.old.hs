...
declTrans top (DeclClass pos cxt cls tvars fdeps methods) =
  cannotTranslate pos "class declaration"
declTrans top (DeclInstance pos cxts cls insts methods) =
  cannotTranslate pos "instance declaration"
declTrans top (DeclPat alt) =
  cannotTranslate (getPos alt) "pattern declaration"
declTrans top (DeclFun pos id0 funs) =
  if not top then
    case funs of
    [Fun [] (Unguarded e) (DeclsParse [])] -> [df]
    other -> cannotTranslate pos "this local definition"
...
declTrans _ (DeclType _ _)       = []
declTrans _ (DeclData _ _ _ _ _) = []
declTrans _ (DeclVarsType _ _ _) = []
declTrans _ d                    = [d]
...
opTrans op arg1 arg2 =
  ...
  ano  -> error ("unexpected operator "++ano++" in opTrans")
  where
