...
mini m n = case ((<=) m n) of { True -> m ; False -> n ; } ;

maxi m n = case ((<=) m n) of { True -> n ; False -> m ; } ;
...
lenAcc acc Nil = acc;
lenAcc acc (Cons x xs) = lenAcc ((+) acc 1) xs;
...
freqSorted ks =
  ...
  Pair (sorted (decreasingFrequencyIn ft) ks) (len ft) ;
...
enKey k = K k (head k) (last k) (len k) ;

hashAssoc (Hash hs hf) = hf ;

findhash mv ks = 
  case hashes mv (len ks) ks (Hash (H Nothing Nothing Nil) Nil) of {
...
hinsert nk h (H lo hi hs) =
    let { newlo = case lo of { Nothing -> h ; Just x -> mini x h } ;
          newhi = case hi of { Nothing -> h ; Just x -> maxi x h } ;
        } in
    case elem h hs of {
    True  -> Nothing ;
