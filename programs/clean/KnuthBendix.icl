...
:: E = Fun (List Int) (List E) | Var (List Int);

:: Weights a b = Weights a b;
...
unlines Nil          =  Nil ;
unlines (Cons s ss)  =  append s (Cons (char '\n') (unlines ss)) ;

emitStr Nil         k  =  k ;
emitStr (Cons c cs) k  =  emit c (emitStr cs k) ;

len Nil          =  0 ;
len (Cons x xs)  =  incr (len xs) ;
...
enumFromTo m n  =  case (<=) m n of {
                   True  -> Cons m (enumFromTo (incr m) n) ;
                   False -> Nil ;
                   } ;
...
concatStrings ss  =  foldr append Nil ss ;
...
showResult (Just trs) sig  =  append (str "\nSuccess\n")
  (showRules sig (map rn trs)) ;
showResult Nothing    sig  =  str "\nFailure\n" ;
...
weights  =  Weights 1 (Cons (Pair (str "0") 1)
  (Cons (Pair (str "+") 0) (Cons (Pair (str "-") 0) Nil))) ;
...
sig  =  Cons (Pair (str "+") (Pair 2 True))  (
        Cons (Pair (str "-") (Pair 1 False)) (
        Cons (Pair (str "0") (Pair 0 False)) Nil )) ;

var x = Var (str x);

fun x y = Fun (str x) y;

eqns  =  Cons (Pair (fun "+" (Cons (fun "0" Nil)
                                   (Cons (var "X") Nil)))
                    (var "X")) (
         Cons (Pair (fun "+" (Cons (fun "-" (Cons (var "X") Nil))
                             (Cons (var "X") Nil) ))
                    (fun "0" Nil)) (
         Cons (Pair (fun "+" (Cons (fun "+" (Cons (var "X") (Cons (var "Y") Nil)))
                             (Cons (var "Z") Nil) ))
                    (fun "+" (Cons (var "X")
                             (Cons (fun "+" (Cons (var "Y") (Cons (var "Z") Nil))) Nil) )))
         Nil )) ;

Start =  case checkEquations eqns sig of {
  	     True  -> emitStr (str "Using equations:\n")
                          (emitStr (showEqns sig eqns)
                                   (emitStr (showResult (complete sig eqns) sig) 0)) ;
  	     False -> emitStr (str "Ill-formed equation\n") 0 ;
         } ;

checkEquations es sig  =  all (both (checkTerm sig)) es ;

checkTerm sig (Var v)     =  True ;
checkTerm sig (Fun f ts)  =  con ((==) (len ts) (arity sig f)) 
...
complete sig trs = completionLoop variables 0 trs Nil ;

completionLoop vs n Nil                    rules  =  Just rules ;
completionLoop vs n (Cons (Pair l r) rest) rules  =
  case (==) n 1000 of {
  True  -> Nothing ;
  False -> case renameNew vs (Pair l r) of {
           Pair newEqn ws -> case orient newEqn of {
                             Nothing      -> Nothing ;
                             Just newRule -> completionWith ws (incr n) rest newRule rules ;
                             } ;
           } ;
  } ;

completionWith vs n rest newRule rules =
  let { cps  =  getCriticalPairs (Cons newRule rules) newRule ; }
  in  case simplifyRules sig newRule rules of {
	    Pair eqs rls -> completionLoop vs n 
...
kbGreaterThan w t s  =
  ...
  in dis (con (inv ((<=) wt ws)) (compareAll (<=) ns nt))
	       (con (con ((==) wt ws) (compareAll (==) nt ns))
...
showTerm sig (Fun f ts)  = 
  ...
  True  -> append (str "(")
           ...
           ( append (str " ") (append f (append (str " ")
	...
	False -> case null ts of {
           ...
           False -> append f
                    ( append (str "(")
                    ( append (concatStrings (intersperse (str ",") (map (showTerm sig) ts)))
                    (str ")") )) ;
           } ;
  } ;

showEqn sig (Pair x y)  =  append (showTerm sig x) (append (str " = ") (showTerm sig y)) ; 

showEqns sig eqns  =  unlines (map (showEqn sig) eqns) ;

showRule sig (Pair x y)  =  append (showTerm sig x) (append (str " -> ") (showTerm sig y)) ; 
...
reducible trs t  =  inv (null (reduce trs t)) ;
...
applyToIndex f n (Cons x xs)  =  case (==) n 0 of {
                                 True -> Cons (f x) xs ;
                                 False -> Cons x (applyToIndex f (decr n) xs) ;
                                 } ;
...
subterm t (Cons p1 pRest)  =  case t of {
                              Fun f ts -> subterm (elemAt ts (decr p1)) pRest ;
                              } ;

placeAt t Nil            u  =  t ;
placeAt t (Cons p pRest) u  =  case u of {
                               Fun f ts ->
                                 Fun f (applyToIndex (placeAt t pRest) (decr p) ts) ;
                               } ; 
...
argPositions ts n  =  map (Cons n) (positions (elemAt ts (decr n))) ;

variables  =  let { variety = Cons Nil (concatMap variations variety) ; }
              in  tail variety ;

variations v  =  map (flip Cons v) (str "ABCDEFGHIJKLMNOPQRSTUVWXYZ") ;
