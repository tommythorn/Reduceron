...
:: Colour     = Black | White ;
:: Piece      :== Pair Colour Kind ;
:: Square     :== Pair Int Int ;
:: Board      = Board (List (Pair Kind Square)) (List (Pair Kind Square)) ;
:: Move       = Move Square (Maybe Piece) (Maybe Piece) ;
:: MoveInFull = MoveInFull Piece Square Move ;
:: Solution   = Solution MoveInFull (List (Pair MoveInFull Solution)) ;
...
mini x y = case (<=) x y of { True -> x ; False -> y ; } ;

maxi x y = case (<=) x y of { True -> y ; False -> x ; } ;

absn n = case (<=) 0 n of { True  -> n ; False -> (-) 0 n ; } ;
...
bishopmoves c sq bd =
	append (moveLine bd c sq (cross (Pair decr incr))) (
	append (moveLine bd c sq (cross (Pair incr incr))) (
	append (moveLine bd c sq (cross (Pair decr decr)))
	       (moveLine bd c sq (cross (Pair incr decr))) )) ;

rookmoves c sq bd =
	append (moveLine bd c sq (cross (Pair decr id))) (
	append (moveLine bd c sq (cross (Pair incr id))) (
	append (moveLine bd c sq (cross (Pair id decr))) 
	       (moveLine bd c sq (cross (Pair id incr))) )) ;

moveLine bd c sq incr = 
	let { incrsq = incr sq ; } in
	case onboard incrsq of {
  True -> case pieceAt bd incrsq of {
		      Nothing -> Cons (Move incrsq Nothing Nothing)
                          (moveLine bd c incrsq incr) ;
		      Just p  -> case sameColour (colourOf p) c of {
				             False -> Cons (Move incrsq (Just p) Nothing) Nil ;
                     True  -> Nil ;
                     } ;
          } ;
	False -> Nil ;
  ...
  Pair xk yk -> 
    case f of {
		...
		Bishop -> dis (con ((==) ((-) x y) ((-) xk yk))
                       (emptyAtAll bd (diagPath minius ((-) xk yk) x xk)))
                  (con ((==) ((+) x y) ((+) xk yk))
                       (emptyAtAll bd (diagPath plus ((+) xk yk) x xk))) ;
		Knight -> dis (con ((==) (absn ((-) x xk)) 2) ((==) (absn ((-) y yk)) 1))
                  (con ((==) (absn ((-) x xk)) 1) ((==) (absn ((-) y yk)) 2)) ;
		Pawn   -> con ((==) (absn ((-) x xk)) 1)
			            ((==) yk (onFor c y )) ;
    } ;
  } ;
...
filePath xk yFrom yTo (Pair x y) =
  let { ylo = (+) (mini yFrom yTo) 1 ; yhi = (-) (maxi yFrom yTo) 1 ; }
  in  con ((==) x xk) (con ((<=) ylo y) ((<=) y yhi)) ; 

rankPath yk xFrom xTo (Pair x y) =
  let { xlo = (+) (mini xFrom xTo) 1 ; xhi = (-) (maxi xFrom xTo) 1 ; }
...
solnOr c n (Pair mif b) other =
	...
	Just rs -> case null rs of {
             True -> case kingincrheck (opponent c) b of {
