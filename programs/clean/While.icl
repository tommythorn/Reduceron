...
invk k a = bool (inv a) k;
andk k a b = bool (and a b) k;

seq f g k = f (comp g k);
comp f g x = f (g x);

:: Val = N Int | V Int | Add Val Val | Sub Val Val;
...
:: BVal = TRUE | FALSE | Eq Val Val | Le Val Val | Neg BVal | And BVal BVal;
...
bval (Neg b) s k = bval b s (invk k);
bval (And a1 a2) s k = seq (bval a1 s) (bval a2 s) (andk k);

:: Stm = Ass Int Val | Skip | Comp Stm Stm
