...
data E = Fun (List Int) (List E) | Var (List Int);
...
main2 =  case checkEquations eqns sig of {
...
completionLoop vs n (Cons (Pair l r) rest) rules  =
  ...
  False -> case renameNew vs (Pair l r) of {
           Pair newEqn ws -> case orient newEqn of {
                             Nothing      -> Nothing ;
                             Just newRule -> completionWith ws (inc n) rest newRule rules ;
                             } ;
           } ;
  } ;
...
kbGreaterThan w t s  =
  ...
  in dis (con (not ((<=) wt ws)) (compareAll (<=) ns nt))
