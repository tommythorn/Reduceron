...
data Map a b    = Empty | Map a b (Map a b);
data Kind       = King | Queen | Rook | Bishop | Knight | Pawn ;
data Colour     = Black | White ;
type Piece      = Pair Colour Kind ;
type Square     = Pair Int Int ;
data Board      = Board (Map Kind Square) (Map Kind Square) ;
data Move       = Move Square (Maybe Piece) (Maybe Piece) ;
...
pieceAtWith sq c n Empty = n ;
pieceAtWith sq c n (Map k s xs) =
...
putPieceAt sq (Pair c k) (Board wkss bkss) =
  case c of {
  White -> Board (Map k sq wkss) bkss ;
  Black -> Board wkss (Map k sq bkss) ;
  } ;
...
concatMap2 f Empty = Nil;
concatMap2 f (Map k s xs) = append (f k s) (concatMap2 f xs);

moveDetailsFor c bd =
  concatMap2 (movesForPiece c bd) (forcesColoured c bd) ;

movesForPiece c bd k s =
