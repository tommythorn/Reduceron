...
bufferMemEn :: Mem -> Bit -> Mem
bufferMemEn mem en =
    mem { memDatIn = map (fde False en) (memDatIn mem) }
...
octoWriteMasked :: (Mem, Mem) -> [Word] -> [Bit] -> Word -> RTL ()
octoWriteMasked (memA, memB) ws mask addr =
  do quadWrite memA (take 4 ws) (take 4 mask) addr
     quadWrite memB (drop 4 ws) (drop 4 mask)
       (addr /+/ (4 `ofWidth` length addr))
