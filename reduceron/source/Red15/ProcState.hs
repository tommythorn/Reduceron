...
--     (stackAMem, stackBMem) <-
--       createMem True stkAddrWidth (dualWide4k []) id
     (stackARaw, stackBRaw) <-
     ...
     stackEnSig <- newSig 1
     [stackEnVal] <- readVar (stackEnSig)

     let stackAMem = bufferMem stackARaw
     let stackBMem = bufferMem stackBRaw
     let stackBuf = groupN wordSize $ map (fde False stackEnVal)
