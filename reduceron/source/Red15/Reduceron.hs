...
import Red15.Mem
import Red15.Bytecode
import Red15.Collect
import Red15.Compat
...
initialise s =  do reg (sp s) <-- maxStkAddr
                   ...
                   octoRead (codeA s, codeB s) (combAddress 0)
                   tick
  ...
  do stackPtr <- readVar (reg (sp s))
     ...
     choose
       ...
       , inv fin --> do tick
                        ...
                        octoRead (codeA s, codeB s)
                                 (take combAddrWidth $ drop 4 t1)
                        tick
       ]
...
handlePrim s t fin =
  do tick
     ...
     octoRead (codeA s, codeB s) (take combAddrWidth $ drop 4 valTop)
...
unwind s =
  do stkPtr <- readVar (reg (sp s))
     ...
     octoRead (codeA s, codeB s) (take combAddrWidth $ drop 4 newTop)
     top s <-- newTop
     reg (sp s) <-- stkPtr'
     tick
...
unfold s tp =
  do let initialAddr = take combAddrWidth (drop 4 tp)
     ...
     ca0 s <-- (initialAddr /+/ combAddress 16)
     addr <- readVar (ca0 s)
     size <- readVar (dec0 s)
     stkPtr <- readVar (reg (sp s))
     ...
     code <- octoRead (codeA s, codeB s) (initialAddr /+/ combAddress 8)
     --code <- octoRead (codeA s, codeB s) initialAddr -- REMOVE!
     code' <- mapM (inst s) code
     --tick  -- REMOVE!
     --zipWithM (<--) (stkBuf s) tops
     --octoRead (codeA s, codeB s) addr
     --ca0 s <-- (addr /+/ combAddress 8) -- REMOVE!
     tick
     ...
     octoRead (codeA s, codeB s) (take combAddrWidth $ drop 4 t)
