...
procState code =
  do spReg     <- newOffsetReg stkAddrWidth 4
     ...
     (heapAMem, heapBMem) <- createMem True addrWidth (dualWide32k [])
     -- addrWidth or gcAddrWidth?
     (gcSpaceAMem, gcSpaceBMem) <- createMem True addrWidth (dualWide12k [])
     (stackAMem, stackBMem) <- createMem False stkAddrWidth (dualWide4k [])
     (astackAMem, astackBMem) <- createMem False stkAddrWidth (dualWide4k [])
     (codeAMem, codeBMem) <- createMem False combAddrWidth (dualWide4k code)
